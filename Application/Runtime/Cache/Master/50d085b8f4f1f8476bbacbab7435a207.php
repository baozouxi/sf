                        ...
                        <div class="addForm">
                            <div class="fields">
                              ...
                              <?php if(is_array($admin_list)): $admin_num = 0; $__LIST__ = $admin_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$admin): $mod = ($admin_num % 2 );++$admin_num;?><ul class="input">
                                        ...
                                        <li><?php echo ($admin["last_login_time"]); ?></li>
                                        <?php if($admin_status <= 1): ?><li><?php echo ($admin['admin_part'] < 1 ? '普通' : '管理员'); ?></li>
                                            <li>无</li>

                                        <?php else: ?>
                                            <li>
                                                <select name="admin_part" data-id=<?php echo ($admin["id"]); ?> >
                                                    <?php if(is_array($admin_part_list)): foreach($admin_part_list as $part=>$item): ?><option value="<?php echo ($part); ?>" <?php echo ($part == $admin['admin_part'] ? 'selected="selected"' :''); ?>  ><?php echo ($item); ?></option><?php endforeach; endif; ?>
                                                </select>
                                            </li><?php endif; ?>
                                            <?php if($admin['is_use'] == 0 ): ?><li><span>冻结</span>
                                                    <a href="/Master/Admin/access/unique/<?php echo ($unique); ?>/do/jiedong/id/<?php echo ($admin["id"]); ?>"><span>解冻</span></a> 
                                                    <a href="/Master/Admin/resetPass/unique/<?php echo ($unique); ?>/id/<?php echo ($admin["id"]); ?>"><span>重置密码</span></a>
                                                </li>
                                            <?php else: ?>
                                                <li><a href="/Master/Admin/access/unique/<?php echo ($unique); ?>/do/dongjie/id/<?php echo ($admin["id"]); ?>"><span>冻结</span></a> 
                                                   <span>解冻</span>
                                                    <a href="/Master/Admin/resetPass/unique/<?php echo ($unique); ?>/id/<?php echo ($admin["id"]); ?>"><span>重置密码</span></a>
                                                </li><?php endif; ?>
