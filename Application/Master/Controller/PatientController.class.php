...
class PatientController extends CommonController 
{
	...
	public function index() 
	{
		...
		$illnessModel  = M('illness');
		$illness = $illnessModel->where(array('hospital_id'=>$hospital_id))->select(); 
	...
	public function info() 
    {
		...
		if (!isset($_GET['tel'])) {
			header('location:/Master/Index/index');
			exit;
		}
		$date = date('Y-m-d');
		...
		$illnessList = $iModel->where(array('hospital_id'=>$_SESSION['hospital_id']))->select();
		$illness = array();
		foreach ($illnessList as $list) {
			$illness[$list['illness_name']] = $list['illness_name'];
		}
		...
		$illness[$patient['illness']] = $patient['illness'];

		//医生数据
		$res = $doctorModel->field('id,name')->where(array('hospital_id'=>$_SESSION['hospital_id']))->select();
	...
	public function addPatient()
    {
		...
		if ($errMsg) {
			...
			$info['0']['hospital_id'] = $_SESSION['hospital_id'];
