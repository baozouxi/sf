...
class SpendingController extends CommonController {
	...
	public function save() {
		...
		if (!isset($_SESSION['status']) || $_SESSION['status'] !== 'admin') {
			if ($info['spending_date'] !== date('Y-m-d')) {
				$errMsg = '修改失败：仅能修改当天记录';
			}
	...
	public function select() {
		...
		if ($errMsg) {
			...
			$map['hospital_id'] = $_SESSION[$prefix]['hospital_id'];
	...
	public function addNext()
	{
		...
		$patients      = $patientModel->where(array('status'=>'0','hospital_id'=>$hospital_id))->select();
				if(count($patients) < 1) $this->success('所有病人已出院','/Home/Index/index');
		...
		if($spendingModel->addAll($res)){
			$this->success('添加成功','/Home/Index/index');
		}
	...
	{	
		...
		$model = M('spending');
		$hospital_name = $_SESSION[C('SESSION_PREFIX')]['hospital_name'];
		$map['patient.hospital_id'] = $_SESSION[C('SESSION_PREFIX')]['hospital_id'];
